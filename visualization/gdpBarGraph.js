...
d3.csv("https://localhost:8888/visualization/tmdb_5000_movies.csv", function(error,data) {
  if(error) console.log("Error: data not loaded!");

  
  data.forEach(function(d) {
    d.movie = d.original_title;
    d.gdp = +d.budget;       // try removing the + and see what the console prints
  ...
  xScale.domain(data.map(function(d) { return d.movie; }) );
  ...
  svg.selectAll('rect')
    ...
    .attr({
      "x": function(d) { return xScale(d.movie); },
    ...
    .style("fill", function(d,i) { return 'rgb(20, 20, ' + ((i * 30) + 100) + ')'});
        ...
        svg.selectAll('text')
            ...
            .attr({
                "x": function(d){ return xScale(d.movie) +  xScale.rangeBand()/2; },
